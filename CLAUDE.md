...
- **Duplicate meal detection** — When logging a meal via `addMealPlan` or `logMeal`, the system checks for duplicates logged within the last 5 minutes using fuzzy matching: name similarity (partial match, case-insensitive) OR matching calories. Returns `duplicate: true` and skips insert. Prevents double-logging when user asks about a meal, coach shows breakdown, then user says "log it" again.
